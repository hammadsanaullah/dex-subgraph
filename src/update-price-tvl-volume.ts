...
export function updateVolume(event: SwapEvent): Volume {
  ...
  // log.debug('My value of getVolumeUSD: {}', [volumeUSD.toString()])
  const bundle = getOrCreateBundle()
...
export function getVolumeUSD(
  ...
  // log.debug('My value is: {}', [tokenAmount0.toString()])
  // log.debug('My second value is: {}', [tokenAmount1.toString()])
  const bundle = getOrCreateBundle()

  const pair = getPair(pairAddress)
  const token0Price = getTokenPrice(pair.token0)
  const token1Price = getTokenPrice(pair.token1)
  // log.debug('My value of token0Price is: {}', [token0Price.derivedNative.toString()])
  // log.debug('My value of token1Price is: {}', [token1Price.lastUsdPrice.toString()])
