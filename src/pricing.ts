...
export function getNativePriceInUSD(): BigDecimal {
  ...
  // log.debug('My value of stablePair is: {}', [stablePair.address.toString()])
    if (stablePair === null) {
  log.debug('Inside if : {}', ["null"])
      continue
    }
  ...
  log.debug('Inside if : {}', ["s"])
    ...
    count = count + 1
  log.debug('My value of count is: {}', [count.toString()])
  ...
  log.debug('My value of weightedPrice is: {}', [weightdPrice.toString()])
    }
  }
  ...
  for (let i = 0; i < token.pairCount.toI32(); ++i) {
    ...
    if (
      ...
      if (token0.decimalsSuccess) {
        ...
        pricedOffPair = pair.id
        mostLiquidity = pair.liquidityNative
        // log.debug('My value of token0Price derivedNative is inside pricing {}', [token0Price.derivedNative.toString()])
        currentPrice = pair.token0Price.times(token0Price.derivedNative)
        // log.debug('My value of currentPrice is: {}', [currentPrice.toString()])
      }
    }
  }
